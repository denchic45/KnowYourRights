...
import com.denchic45.knowyourrights.ui.adapter.SingleChoiceAdapterDelegate
...
class SingleChoiceFragment :
    BaseFragment<SingleChoiceViewModel, FragmentChoiceContainerBinding>(R.layout.fragment_choice_container) {

    override val binding: FragmentChoiceContainerBinding by viewBinding(
        FragmentChoiceContainerBinding::bind
    )

    override val viewModel: SingleChoiceViewModel by viewModels { viewModelFactory }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        with(binding.root) {
            val answersAdapter = adapter {
                ...
                onClick { position ->
                    ...
                    lifecycleScope.launchWhenStarted {
                        playerViewModel.onSingleAnswerSelect(position)
                    }
                }
            }
            ...
            playerViewModel.showActualAnswer
                .take(1)
                .map { it as QuizPlayerViewModel.ActualAnswer.SingleActualAnswer }
